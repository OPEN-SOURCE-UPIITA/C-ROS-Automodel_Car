...
class DetectorMultiCarrilNode(Node):
    def __init__(self):
        ...
        self.right_fit = None   
        self.lane_width_pixels = 250.0 # Ancho de un solo carril en Bird's Eye
    ...
    def image_callback(self, msg):
        ...
        # Creamos una copia de la imagen binaria para ir "quemando" zonas usadas
        ...
        for window in range(nwindows):
            ...
            # 1. VENTANA DERECHA (Ahora tiene la prioridad máxima)
            if curr_r is not None:
                ...
                # Buscamos puntos solo en lo que queda disponible
                zona_r = img_disponible[win_y_low:win_y_high, xr_low:xr_high]
                
                if np.any(zona_r > 0):
                    # Obtenemos los índices globales de los píxeles blancos en este cuadro
                    ...
                    if minpix < len(ids) < maxpix:
                        ...
                        # --- EXCLUSIÓN: Borramos esta zona para que nadie más la use ---
                        img_disponible[win_y_low:win_y_high, xr_low:xr_high] = 0

            # 2. VENTANA CENTRAL (Prioridad media)
            if curr_c is not None:
                ...
                if np.any(zona_c > 0):
                    ...
                    if minpix < len(ids) < maxpix:
                        ...
                        # --- EXCLUSIÓN: Borramos esta zona para la línea izquierda ---
                        img_disponible[win_y_low:win_y_high, xc_low:xc_high] = 0

            # 3. VENTANA IZQUIERDA (Se queda con lo que sobre)
        ...
        y_eval = h - 50 # Evaluar cerca de la base del robot
        ...
        if self.left_fit is not None and self.center_fit is not None:
            self.draw_lane_area(out_img, self.left_fit, self.center_fit, (0, 255, 0)) # Verde
        if self.center_fit is not None and self.right_fit is not None:
            self.draw_lane_area(out_img, self.center_fit, self.right_fit, (255, 255, 0)) # Cian
