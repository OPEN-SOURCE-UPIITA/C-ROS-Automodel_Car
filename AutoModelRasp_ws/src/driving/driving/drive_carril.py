...
# ==============================================================================
# CONTROL DE CARRIL + FRENADO MODULAR (Optimizado para Motores Físicos)
# ==============================================================================
...
class DriveCarrilNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('velocidad_minima', 60)       # Velocidad mínima absoluta (evita que el motor se congele)
        ...
        self.estado_vehiculo = "CONDUCIENDO" # Estados: CONDUCIENDO, DETENIDO, IGNORANDO
        ...
        self.get_logger().info("Nodo Drive Carril (PD + Frenado Modular + Anti-Deadband) Iniciado")

    def distancia_callback(self, msg):
        """Actualiza la memoria con la distancia reportada por la cámara."""
        self.distancia_senal = msg.data

    def evaluar_estado_frenado(self):
        """
        Máquina de estados independiente.
        Retorna: True si el motor debe detenerse, False si puede acelerar.
        """
        ...
        if self.estado_vehiculo == "CONDUCIENDO":
            ...
            # Mantener freno hasta que pase el tiempo configurado
            if dt_estado >= t_paro:
                self.estado_vehiculo = "IGNORANDO"
                self.tiempo_cambio_estado = ahora
                self.distancia_senal = 999.0 # Reset artificial para evitar falsos positivos
                self.get_logger().info("Tiempo de STOP cumplido. Arrancando (Modo Ignorar)...")
                return False
            return True

        elif self.estado_vehiculo == "IGNORANDO":
            # Periodo de gracia para que el auto rebase la señal y no se vuelva a frenar
            if dt_estado >= t_ign:
                self.estado_vehiculo = "CONDUCIENDO"
                self.distancia_senal = 999.0
    ...
    def error_callback(self, msg):
        ...
        target_servo = self.servo_centro - (P + D)
        target_servo = int(np.clip(target_servo, 1110, 1740)) # Límites mecánicos

        # Suavizado de giro (Slew Rate) para evitar "latigazos"
        max_step = base_step + int(abs(error_actual) * 20)
        diff = target_servo - self.last_servo_value
        ...
        # Reduce la velocidad proporcionalmente al error, pero limitando al piso (vel_min)
        ...
        if self.evaluar_estado_frenado():
            speed_dc = 0 # Freno total si la señal de STOP está activa
        else:
...
def main(args=None):
    ...
    try:
        ...
        # Freno de emergencia si se apaga el nodo
        stop = MotorCommand()
        stop.dir_dc = 0
        stop.speed_dc = 0
        stop.dir_servo = 1500
...
# ==============================================================================
# CONTROL DE CARRIL + FRENADO MODULAR + BOTÓN START/STOP EN RQT
# ==============================================================================
