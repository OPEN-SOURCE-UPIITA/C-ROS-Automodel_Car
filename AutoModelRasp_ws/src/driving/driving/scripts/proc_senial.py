...
# Inicializar parámetros (valores por defecto)
        self.lower_red1 = np.array([0, 100, 100])
        self.upper_red1 = np.array([10, 255, 255])
        self.lower_red2 = np.array([170, 100, 100])
        self.upper_red2 = np.array([180, 255, 255])
    ...
    def params_callback(self, msg):
        """Recibe parámetros de color desde el tópico"""
        try:
            if len(msg.data) == 12:
                # Rango rojo 1
                self.lower_red1 = np.array(msg.data[0:3])
                self.upper_red1 = np.array(msg.data[3:6])
                # Rango rojo 2
                self.lower_red2 = np.array(msg.data[6:9])
                self.upper_red2 = np.array(msg.data[9:12])
            
                self.get_logger().info(f"Nuevos parámetros recibidos:")
                self.get_logger().info(f"  Rojo1: lower={self.lower_red1}, upper={self.upper_red1}")
                self.get_logger().info(f"  Rojo2: lower={self.lower_red2}, upper={self.upper_red2}")
        except Exception as e:
            self.get_logger().error(f"Error al procesar parámetros: {e}")
